...
long largestProductInAGrid(vector<vector<int>> &arreglo) {
  unsigned long largestProduct = 0;
  for (unsigned int y = 0; y < arreglo.size(); y++) {
    for (unsigned int x = 0; x < arreglo.size(); x++) {
      if (x + 3 < arreglo.size()) {
        unsigned int current = arreglo[x][y] * arreglo[x + 1][y] *
                               arreglo[x + 2][y] * arreglo[x + 3][y];
        largestProduct = (largestProduct < current) ? current : largestProduct;
      }
      if (y + 3 < arreglo.size()) {
        unsigned int current = arreglo[x][y] * arreglo[x][y + 1] *
                               arreglo[x][y + 2] * arreglo[x][y + 3];
        largestProduct = (largestProduct < current) ? current : largestProduct;
      }
      if (x + 3 < arreglo.size() && y + 3 < arreglo.size()) {
        unsigned int current = arreglo[x][y] * arreglo[x + 1][y + 1] *
                               arreglo[x + 2][y + 2] * arreglo[x + 3][y + 3];
        largestProduct = (largestProduct < current) ? current : largestProduct;
      }
      if (x + 3 < arreglo.size() && y >= 3) {
        unsigned int current = arreglo[x][y] * arreglo[x + 1][y - 1] *
                               arreglo[x + 2][y - 2] * arreglo[x + 3][y - 3];
        largestProduct = (largestProduct < current) ? current : largestProduct;
      }
...
int main() {
  ...
  cout << largestProductInAGrid(grid) << endl;
  return 0;
}
